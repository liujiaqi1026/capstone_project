...
def multicar_optimize(railcar_list: List[Railcar], tie_list: List[Tie], bundle_v: int, bundle_h: int, tie_width: float,
    ...
    for railcar in railcar_list:
        ...
        # res = optimize(railcar_list_tmp, tie_list, bundle_v, bundle_h, tie_width, tie_thickness, weight_diff)
        try:
            res = optimize(railcar_list_tmp, tie_list, bundle_v, bundle_h, tie_width, tie_thickness, weight_diff)
        ...
        #         tie_original = tie_list_copy[i]
        #         tie_new = tie_list[i]
        #         q_diff = tie_original.quantity - tie_new.quantity
        #
        #         tie_list_copy[i].quantity = tie_list[i].quantity - q_diff
        #
        #         if tie_list_copy[i].quantity < 0:
        #             check = False
        #     if check:
        #         result.append(copy.deepcopy(res_x))
        #         tie_list = tie_list_copy
        #
        #     print("q")
    ...
    for i in range(len(result)):
        for car in result[i]:
            ...
            for side in car:
                ...
                for layer in side:
                    # print(layer)
                    # print(np.array(layer).astype(int))
